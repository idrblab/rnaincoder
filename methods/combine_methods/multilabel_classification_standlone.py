...
def calculate_twolabel_auc(true_label_, pre_label, logits_, class_list, path, n_name,
    ...
    tpr["macro"] = mean_tpr
    roc_auc["macro"] = auc(fpr["macro"], tpr["macro"])
    plt.plot(fpr["macro"], tpr["macro"], color='black', marker='.', markersize=10, lw=lw, linestyle='-',
             label='AUC = %0.3f' % (roc_auc["macro"]))
...
def train_DNNModel(targetpath, labelpath, Result_path, modelnm, Epochs=50, learnrante=0.001, batch_size=32, shape1=13,
    ...
    if modelnm == 'DNN':
        ...
        # RNA_CODING_LENGTH = 340
        # model = conjoint_sae(encoders_pro, encoders_rna, PRO_CODING_LENGTH, RNA_CODING_LENGTH)
...
def evaluate_DNN(alldata, datapath, png_path, n_name, labelpath=None, label=None, shape1=128, shape2=32, dropout=0.2):
    if label == 1:
        ...
        confusion = metrics.confusion_matrix(label, logits_)
        n_nameauc = n_name
        n_name = n_name[:-4]
        plot_confusion_matrix(confusion, class_, png_path, n_name)

        rnaheatmap1(data, label, png_path, sename='before_' + str(n_name), title = 'before',method='ward', metric='euclidean')
        plot_clustering_matplotlib(data, label, png_path, str(n_name) + '_before', ' before Model Learning')

        rnaheatmap1(m_f, label, png_path, sename='after_' + str(n_name), title = 'after', method='ward', metric='euclidean')
        plot_clustering_matplotlib(m_f, label, png_path, str(n_name) + '_after', ' after Model Learning')
        ...
        return neuralfeatures, aucvalue, ACC, MCC, precision, f1, recall, fbeta
    else:
